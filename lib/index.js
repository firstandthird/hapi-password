  ...
  server.auth.scheme(pluginOptions.schemeName, (authServer, options) => {
    ...
    return {
      authenticate: (request, reply) => {
        ...
        if (cookie) {
          const credentials = _.find(options.password, (o) => {
            return o.encryptedPassword === cookie;
          });
          if (credentials) {
            // return the credentials, minus the encryptedPassword:
            return reply.continue({ credentials: _.omit(credentials, ['encryptedPassword']) });
          }
